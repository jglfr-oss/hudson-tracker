...
// dedupe (keeping the higher-ranked of any collision), sort, and cap.
// The score always decides dedup winners and which items make the cut; when
// `sortByDate` is set the surviving items are then DISPLAYED newest-first
// (used for News), otherwise they stay in score order (used for Community).
function buildList(items, scorer, now, { limit = MAX_ITEMS_PER_TAB, sortByDate = false } = {}) {
  ...
  if (sortByDate) {
    ...
    top.sort((a, b) => (b.publishedAt ?? 0) - (a.publishedAt ?? 0));
  }

  // Drop the internal fields from the public payload.
  return top.map(({ _rank, credibility, ...rest }) => rest);
}
...
export function buildPulse(results, now = Date.now()) {
  ...
  const news = buildList(newsItems, newsScore, now, { sortByDate: true });
