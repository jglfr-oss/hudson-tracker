...
// Google News RSS titles look like "Headline - Publisher Name". Split them so
// the card shows the real publisher as the source. Pure — testable.
export function splitGoogleNewsTitle(title) {
  const t = String(title || "");
  const idx = t.lastIndexOf(" - ");
  // Publisher suffixes are short; a long tail is more likely part of the headline.
  if (idx > 10 && t.length - idx - 3 <= 60) {
    return { headline: t.slice(0, idx).trim(), publisher: t.slice(idx + 3).trim() };
  }
  return { headline: t, publisher: null };
}
...
export function normalizeFeedRecords(records, feedMeta, fetchedAt = Date.now()) {
  const out = [];
  const isGoogleNews = feedMeta.platform === "googlenews";
  for (const r of records || []) {
    const text = `${r.title || ""} ${r.description || ""}`;
    if (!isType1Relevant(text)) continue;
    let title = r.title;
    let source = feedMeta.source;
    let excerpt = r.description;
    if (isGoogleNews) {
      const { headline, publisher } = splitGoogleNewsTitle(r.title);
      title = headline;
      if (publisher) source = publisher;
      // Google News descriptions just repeat the linked headline — not a real excerpt.
      excerpt = "";
    }
    const item = makeItem(
      {
